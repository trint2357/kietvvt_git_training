...
function createTimeStamp(ms) {
  ...
  return `${dur.hours() > 9 ? dur.hours() : "0" + dur.hours()}:${
...
export default function awsTranscribeToVtt(
  event,
  { characterPerLine = 40, maxLines = 2, maxTimeInMs = 1000 } = {}
) {
  ...
  return (
    ...
    event.results.items
      .map((e) => ({
        ...
        characterCount: e.alternatives[0].content
          ? e.alternatives[0].content.length
          : 0,
      ...
      .reduce((acc, cur) => {
        ...
        if (cur.type === "pronunciation") {
          const shouldMerge = wordsToLineMergeRules.reduce(
            (item, rule) => (item ? rule(last, cur) : false),
            true
          );
      ...
      .reduce((acc, cur) => {
        ...
        const shouldMerge = multilineMergeRules.reduce(
          (item, rule) => (item ? rule(last, cur) : false),
          true
        );
      ...
      .reduce((acc, cur, idx) => {
        return [
          ...acc,
          `${idx + 1}\n` +
            `${createTimeStamp(cur.from)} --> ${createTimeStamp(
              cur.to
            )} line:95%\n` +
            `${cur.content}\n\n`,
        ];
